module MultiSenderMailerPatch
  ...
  module InstanceMethods
    ...
    def mail_with_helpdesk(headers={}, &block)
      ...
      if project.present?
        sender_email_field = ProjectCustomField.find_by_name "helpdesk-sender-email"
        if sender_email_field.try(:id).present?
          sender_email = project.custom_field_value(sender_email_field.try(:id))
